...
function ViewCoursesStudent() {
  ...
  useEffect(() => {
    axios.get(`http://127.0.0.1:3000/getStudentCourses?id=${id}`)
      .then(res => {
        if (res.data.Status === 'Success') { 
          setStudentCourse(res.data.Result);
        } else {
      ...
      <Grid container spacing={2} justifyContent='center'>								 
        {studentCourse.map((course, index) => (
          <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
            <Link to={`/studentdashboard/viewcoursedetailsstudent/${course.Id}`} style={{ textDecoration: 'none' }}>
